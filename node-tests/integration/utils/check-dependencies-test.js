...
describe('checkDependencies()', function() {
  ...
  beforeEach(function() {
    ...
    fs.writeFileSync(path.join(electronProjectPath, 'package.json'), JSON.stringify({
      dependencies: {
        'fake-package': '^1.0.0'
      }
    }));
  ...
  it('works with correct dependencies', async function() {
    ...
    fs.writeFileSync(path.join(nodeModulesPath, 'fake-package', 'package.json'), JSON.stringify({
      name: 'fake-package',
      version: '1.0.2'
    }));
