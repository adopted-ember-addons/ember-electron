...
module.exports = Command.extend({
  ...
  _copyFiles({ tmpPath }, logger = new Logger(this)) {
    ...
    let trees = mergeTrees([
      new Funnel(path.join(basePath, 'public')),
      new Funnel(path.join(basePath, `public-${options.platform}`))
    ]);

    logger.message(trees);
