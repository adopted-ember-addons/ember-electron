...
module.exports = Command.extend({
  ...
  _copyFiles({ platform = process.platform, tmpPath }, logger = new Logger(this)) {
    ...
    if (ee['copy-files']) {
      logger.error('The "copy-files" configuration option is no longer used. ' +
    ...
    const pathDepthAdjustingRegEx = new RegExp(`^ember-electron/public(-${platform})?/`);

    return all(patterns.map((pattern) => {
      return glob(pattern)
        .then((files) => all(files.map((file) => {
          const pathDepthAdjustedFile = file.replace(pathDepthAdjustingRegEx, '');
          logger.message(`Copying ${file} -> ${pathDepthAdjustedFile}`);

          return copy(
            path.resolve(this.project.root, file),
            path.resolve(tmpPath, pathDepthAdjustedFile)
          );
        })));
    }));
  },
