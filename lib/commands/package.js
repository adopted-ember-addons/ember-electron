...
module.exports = Command.extend({
  ...
  recompileDependencies: function (options) {
    return new Promise((resolve, reject) => {
      ...
      const target = `${this.project.root}/tmp/electron-build-tmp/`
      const ee = this.project.pkg['ember-electron'] || {}
      const arch = options.arch || ee.arch || process.arch

      if (!electronVersion) {
        this.ui.writeLine('')
        this.ui.writeLine(chalk.red('Could not determine Electron Prebuilt version for native compilation.'))
        this.ui.writeLine('Please reinstall electron or electron-prebuilt.')
        this.ui.writeLine('')
        return reject()
      }
      ...
      rebuild(target, electronVersion, arch)
        .then(() => resolve())
        .catch((e) => {
          this.ui.writeLine('')
